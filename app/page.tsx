        ...
        <div className="mx-auto max-w-6xl px-4 py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center">
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-white border border-[var(--color-border)] px-3 py-1 text-xs font-medium text-slate-700">
              ...
              <span>Sinds {business.foundedYear} — al {new Date().getFullYear() - business.foundedYear}+ jaar uw installateur</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-slate-900 mb-4">
              {business.tagline}
            </h1>
            <p className="text-lg text-slate-600 mb-8 leading-relaxed max-w-xl">
              {business.name} staat voor vakwerk, vaste prijs vooraf en snelle service. {trade.typeCapital} in {business.address.city} en omgeving — al meer dan {new Date().getFullYear() - business.foundedYear} jaar het vertrouwde adres voor installatietechniek.
            </p>
          ...
          <div className="relative rounded-2xl border border-[var(--color-border)] bg-white shadow-lg overflow-hidden">
            {/* Experience showcase — voor lang-gevestigd familiebedrijf */}
            <div className="bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary)]/80 text-white px-6 py-5">
              <div className="text-xs uppercase tracking-wider opacity-80 mb-1">Familiebedrijf sinds {business.foundedYear}</div>
              <div className="flex items-baseline gap-2">
                <div className="text-4xl font-bold leading-none">{new Date().getFullYear() - business.foundedYear}+</div>
                <div className="font-medium opacity-90">jaar ervaring in Amsterdam</div>
              </div>
              <div className="mt-3 text-sm opacity-90">Vader &amp; zonen — installatietechniek, sanitair &amp; verwarming</div>
            </div>
            <div className="p-6">
              <div className="text-sm text-slate-500 mb-2">Klanten aan het woord:</div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-0.5">
                  {Array.from({ length: reviews[0]?.rating ?? 5 }).map((_, i) => (
